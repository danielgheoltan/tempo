...
$logRowTemplate = <<<HTML
    <tr class="log" data-started="" data-synced="false">
        <td data-for="status_db">
            <div class="icon-status icon-status--db"
                 data-name="status_db"
            >
        ...
        <td data-for="status_jira">
            <div class="icon-status icon-status--jira tooltip"
                 data-name="status_jira"
                 data-title="{$_ENV['i18n']('Save')}"
                 onclick="APP.ButtonPush.onclick(event)"
...
<?php foreach ($period as $dt): ?>
    ...
    $isFreeDay = in_array($date1, $freeDays);
    ...
    $stop = clone $start;
    if (isset($timesheet[$date1]) && is_array($timesheet[$date1])) {
        foreach ($timesheet[$date1] as $k => $v) {
    ...
    <table class="<?= $class ?>" data-started="<?= $date2 ?>" data-fulfilled="<?= $fulfilled ?>" spellcheck="false">
        ...
        <tbody>
            <?php $j = 0; ?>
            <?php if (isset($timesheet[$date1]) && is_array($timesheet[$date1])): ?>
                <?php foreach ($timesheet[$date1] as $k => $v): ?>
                    <tr class="log"
                        data-index="<?= $v['index'] ?>"
                        data-started="<?= $date2 ?>"
                        <?php if ($v['id'] > 0): ?>
